...
class SearchResultsListAdapter(
        ...
        when(holder){
            is SearchDataViewHolder -> {
                ...
                analysisBtn.setOnClickListener {
                    ...
                    topFragmentHolder?.openSearchAnalysis(stockItems[position])
                }
            }
        }
    }
    ...
    fun bindStockItems(newStockItems : List<StockItem>) {
        Timber.d("Binding new search results with count: %s", newStockItems.count())
        stockItems = newStockItems
        notifyDataSetChanged()
    }
}

private class SearchDataViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView){
